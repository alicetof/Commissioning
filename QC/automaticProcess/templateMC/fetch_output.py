...
def get_ccdb_obj(ccdb_path,
    ...
    if musthave_in_metadata is not None and download_mode == "wget":
        url = f"{host}/latest/{ccdb_path}"
        #print(url)
        #print("!!!!!!!!!!!!!!!!!!!!!!!!!!")

        if "PeriodName" in musthave_in_metadata:
            url += "/PeriodName={}".format(musthave_in_metadata["PeriodName"])
        if "RunNumber" in musthave_in_metadata:
            url += "/RunNumber={}".format(musthave_in_metadata["RunNumber"])
        if "PassName" in musthave_in_metadata:
            url += "/PassName={}".format(musthave_in_metadata["PassName"])
        rqst = requests.get(url)
        timestamp_index = rqst.text.split().index("Validity:")+1
        timestamp = int(rqst.text.split()[timestamp_index])
        #print(timestamp)
...
if __name__ == "__main__":
    ...
    do_meta_dict(args.passname, "PassName")
