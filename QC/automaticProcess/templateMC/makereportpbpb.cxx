...
void makereportpbpb(TString period = "LHC23zs", TString pass = "apass1")
{
    ...
    // Color you life
    int col[] = {kBlack, kRed, kBlue, kGreen, kOrange, kMagenta, kCyan, kGray + 1, kViolet, kTeal + 2, kYellow,
                 kBlack, kRed, kBlue, kGreen, kOrange, kMagenta, kCyan, kGray + 1, kViolet, kTeal + 2, kYellow,
                 kBlack, kRed, kBlue, kGreen, kOrange, kMagenta, kCyan, kGray + 1, kViolet, kTeal + 2, kYellow,
                 kBlack, kRed, kBlue, kGreen, kOrange, kMagenta, kCyan, kGray + 1, kViolet, kTeal + 2, kYellow};
    int marker[] = {20, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20,
    ...
    for (int n_run = 0; n_run < nruns; n_run++)
    {
        ...
        if (f[7][n_run]) hDecodingE[n_run] = (TH2D *)f[7][n_run]->Get("ccdb_object");
        else hDecodingE[n_run] = 0x0;

        if (f[8][n_run]) hOrbitVsCrate[n_run] = (TH2D *)f[8][n_run]->Get("ccdb_object");
        else hOrbitVsCrate[n_run] = 0x0;
    ...
    int totch = 1;
    if (hHitMap[0]) {
        totch = (hHitMap[0]->GetNbinsX() * hHitMap[0]->GetNbinsY());
    }
    ...
    */
        ...
        if (hHitMap[i]) {
            ...
            for (int isect = 1; isect <= hHitMap[i]->GetNbinsX(); isect++){
                for (int istrip = 1; istrip <= hHitMap[i]->GetNbinsY(); istrip++){
                    ...
                    if (hHitMap[i]->GetBinContent(isect, istrip) > 100) countchannels[i]++;
                }
            }
        } else {
    ...
    for (int i = 0; i < nruns; i++){
        ...
        if (hOrbitVsCrate[i]) {
            ...
            drmcorr[i] = hOrbitVsCrate[i]->Integral(1, 72, 1, 3) / hOrbitVsCrateClone[i]->Integral(1, 72, 1, 3);
        } else {
            drmcorr[i] = 1;
        }
    ...
    TCanvas* ceff[3];
    TString tracktypebis[3] = {"ITS-TPC(-TRD) tracks", "TPC tracks", "TPC-TRD tracks"};
    ...
    for (int itracktype = 0; itracktype < 3; itracktype++) {
        ...
        for (int i = 0; i < nruns; i++) {
            ...
            hEffPt[0 + itracktype][i] = new TH1F(Form("hEffPt_%s_%s", tracktypebis[itracktype].Data(), run.at(i).Data()), "", neffbins, 0., 20);
            ...
            for (int ibin = 1; ibin <= neffbins; ibin++){
                ...
                hEffPt[0 + itracktype][i]->SetBinContent(ibin, hEff[0 + itracktype][i]->GetEfficiency(ibin));
                hEffPt[0 + itracktype][i]->SetBinError(ibin, hEff[0 + itracktype][i]->GetEfficiencyErrorUp(ibin));
            }
            ...
            histo_hestetics(hEffPt[0 + itracktype][i], col[i], marker[i]);
        }
        leg->Draw();
        label->DrawLatex(0.3, 0.95, Form("%s %s", period.Data(), pass.Data()));
        label->DrawLatex(0.75, 0.95, Form("%s", tracktypebis[itracktype].Data()));
    ...
    for (int itracktype = 0; itracktype < 3; itracktype++) {
        ...
        htrendCorr_ch[itracktype] = new TH1F(Form("htrendCorr_ch_%s", tracktypebis[itracktype].Data()), ";;Correction", nruns, 0., nruns);
        htrendCorr_trm[itracktype] = new TH1F(Form("htrendCorr_trm_%s", tracktypebis[itracktype].Data()), ";;Corr for decoding errors", nruns, 0., nruns);
        htrendCorr_drm[itracktype] = new TH1F(Form("htrendCorr_drm_%s", tracktypebis[itracktype].Data()), ";;Corr for lost orbits", nruns, 0., nruns);
        htrendCorr_tot[itracktype] = new TH1F(Form("htrendCorr_tot_%s", tracktypebis[itracktype].Data()), ";;Corr TOT", nruns, 0., nruns);
    ...
    //Corrections
    TLegend *legcorr = new TLegend(0.2, 0.2, 0.6, 0.38);
    ...
    for (int itracktype = 0; itracktype < 3; itracktype++) {
        ...
        htrendCorr_ch[itracktype]->GetYaxis()->SetRangeUser(0., 1.1);
   ...
   cchi[0]->SaveAs(Form("Summary_%s_%s.pdf", period.Data(),pass.Data()));
    ...
    TLatex *label0 = new TLatex();
    label0->SetNDC();
    label0->SetTextFont(42);
    label0->SetTextColor(kRed);
    label0->SetTextSize(0.03);
    int label0counter = 0;

    TH1D* hempty = new TH1D("hempty", "", 100, -8, 8);
    hempty->SetStats(0);
    hempty->GetYaxis()->SetRangeUser(0, 1.1);
    hempty->GetXaxis()->SetTitle("#Delta BC");
    hempty->GetYaxis()->SetTitle("Norm Counts");
    ...
    for (int i = 0; i < nruns; i++)
    {
        ...
        hDeltaBCT0TOF[i]->GetYaxis()->SetRangeUser(0, 2*hDeltaBCT0TOF[i]->GetMaximum());
        ...
        cdeltaBC_summary->cd();
        //hDeltaBCT0TOF[i]->DrawNormalized("HIST P");
        if (TMath::Abs(hDeltaBCT0TOF[i]->GetMean()) > 0.5) {
            if (label0counter == 0)
                label0->DrawLatex(0.13, 0.68, "Warning!");
            label0->DrawLatex(0.13, 0.65 - label0counter * 0.03, Form("Run %s shifted", run.at(i).Data()));
            label0counter++;
        }
        if (i == 0)
            hempty->Draw();
        hDeltaBCT0TOF[i]->DrawNormalized("HIST SAME");
    }
    leg->Draw();
    ...
    TCanvas *cTrkTime_tof[nruns];
    TCanvas *cTrkTime_tof_summary = new TCanvas("cTrkTime_tof_summary", "", 1000, 1000);
    canvas_multiple(cTrkTime_tof_summary, nruns);
    for (int i = 0; i < nruns; i++)
    {
        cTrkTime_tof[i] = new TCanvas(Form("cTrkTime_tof_%s", run.at(i).Data()), "", 1000, 900);
        canvas_hestetics_single(cTrkTime_tof[i]);
        cTrkTime_tof[i]->SetLogz();
        hTrkTime[i]->SetStats(0);
        hTrkTime[i]->SetTitle("#Delta t track-TOF time");
        hTrkTime[i]->Draw("colz");

        cTrkTime_tof[i]->SaveAs(Form("images/TrkTime_t0TOF_%s.png", run.at(i).Data()));
        cTrkTime_tof[i]->SaveAs(Form("Summary_Run%s_%s_%s.pdf", run.at(i).Data(), period.Data(), pass.Data()));

        cTrkTime_tof_summary->cd(i + 1);
        cTrkTime_tof_summary->cd(i + 1)->SetLogz();
        hTrkTime[i]->Draw("colz");
        label->DrawLatex(0.48, 0.85, Form("Run %s", run.at(i).Data()));
        label->DrawLatex(0.48, 0.8, Form("ITS-TPC(-TRD) tracks"));
    }
    cTrkTime_tof_summary->SaveAs(Form("Summary_%s_%s.pdf", period.Data(), pass.Data()));
    ...
    TFile *write2 = new TFile(Form("MatchEff_trending_%s_%s.root", period.Data(), pass.Data()), "RECREATE");
    for (int i = 0; i<3; i++){
        htrendEff[i]->Write();
    ...
    TDirectory *dir2 = write2->mkdir("MatchEfficienciesITSTPCTRD");
    dir2->cd();
    for (int i = 0; i < nruns; i++) {
        hEffPt[0][i]->Write();
    }
...
void canvas_multiple(TCanvas* c, int n){
    ...
    if (n==26) c->Divide(5,6);
    if (n==27) c->Divide(5,6);
    if (n==28) c->Divide(5,6);
    if (n==29) c->Divide(5,6);
    if (n==30) c->Divide(5,6);
    if (n==31) c->Divide(6,6);
    if (n==32) c->Divide(6,6);
    if (n==33) c->Divide(6,6);
    if (n==34) c->Divide(6,6);
    if (n==35) c->Divide(6,6);
    if (n==36) c->Divide(6,6);
    if (n==37) c->Divide(6,7);
    if (n==38) c->Divide(6,7);
    if (n==39) c->Divide(6,7);
    if (n==40) c->Divide(6,7);
    if (n==41) c->Divide(6,7);
    if (n==42) c->Divide(6,7);
    if (n==43) c->Divide(7,7);
    if (n==44) c->Divide(7,7);
    if (n==45) c->Divide(7,7);
    if (n==46) c->Divide(7,7);
    if (n==47) c->Divide(7,7);
    if (n==48) c->Divide(7,7);
    if (n==49) c->Divide(7,7);
    if (n==50) c->Divide(7,8);
    if (n==51) c->Divide(7,8);
    if (n==52) c->Divide(7,8);
    if (n==53) c->Divide(7,8);
    if (n==54) c->Divide(7,8);
    if (n==55) c->Divide(7,8);
    if (n==56) c->Divide(7,8);
    if (n==57) c->Divide(8,8);
    if (n==58) c->Divide(8,8);
    if (n==59) c->Divide(8,8);
    if (n==60) c->Divide(8,8);
    if (n==61) c->Divide(8,8);
    if (n==62) c->Divide(8,8);
    if (n==63) c->Divide(8,8);
    if (n==64) c->Divide(8,8);
}
